...
class MLIRVerifier:
    ...
    def verify_passes(self, passes, output_dir=None, output_info=True):
        ...
        if output_dir is not None:
            ...
            with open(os.path.join(output_dir, f"input_module.mlir"), "w") as f:
                f.write(mlir_text)
                print(
                    f"Input module written to: {os.path.join(output_dir, f'input_module.mlir')}"
                )
    ...
    def verify_translate(
        self, mlir_text, output_dir=None, output_filename=None, output_info=True
    ):
        """
        ...
        translate_result = subprocess.run(
            translate_cmd, input=mlir_text, capture_output=True, text=True
        )
        if translate_result.returncode != 0:
            ...
            if output_dir is not None:
                llvm_filename = (
                    output_filename if output_filename is not None else "output.ll"
                )
    ...
    def verify(self, passes, output_dir=None, output_filename=None):
        ...
        if mlir_text is None:
            return False
        return self.verify_translate(
            mlir_text, output_dir=output_dir, output_filename=output_filename
        )
    ...
    def find_effective_pass_sequence(self, output_dir="fuzz_seq_results"):
        ...
        while available_passes:
            ...
            for p in available_passes[:]:
                ...
                if (
                    mlir_text is not None
                    and mlir_text.strip() != current_mlir.strip()
                    and mlir_text != ""
                ):
                    sequence.append(p)
                    ...
                    progress = True
                    with open(
                        os.path.join(output_dir, f"step_{len(sequence)}_{p}.mlir"), "w"
                    ) as f:
                        f.write(current_mlir)
                    if verifier.verify_translate(
                        current_mlir,
                        output_dir=output_dir,
                        output_filename=f"finalized_llvm.ll",
                        output_info=False,
                    ):
