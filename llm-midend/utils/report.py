...
def egraph_to_svg(egraph: EGraph) -> HTML:
    ...
    svg_str = svg_raw.decode("utf-8") if isinstance(svg_raw, bytes) else svg_str

    svg_data = svg_str

    # Escape the SVG data properly for JavaScript
    svg_escaped = svg_data.replace("\\", "\\\\").replace("`", "\\`").replace("$", "\\$")
...
class Report(ReportInterface):
    ...
    def _render_nested_report(self, nested_report):
        ...
        nested_html = nested_report._generate_nested_html(parent_id=self.report_id)
    ...
    def _display_terminal(self, indent_level=0, _stored_contents=None):
        ...
        contents_to_use = _stored_contents or getattr(self, "_original_contents", {})
        ...
        for i, pane in enumerate(self.panes, 1):
            ...
            if indent_level == 0:
                ...
                print(f"{content_indent}{DIM}{'─' * (len(pane['title']) + 10)}{RESET}")
            else:
                print(
                    f"{content_indent}{BOLD}{YELLOW}[{i}] {expanded_indicator} {pane['title']}{RESET}"
                )
                print(f"{content_indent}{DIM}{'─' * (len(pane['title']) + 8)}{RESET}")
            ...
            if isinstance(original_content, Report):
                ...
                original_content._display_terminal(indent_level + 1, contents_to_use)
            else:
                # Regular content processing
                if original_content is not None:
                    content = str(original_content)
                else:
                    content = pane.get("fallback_content", str(pane["content"]))
