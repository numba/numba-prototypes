...
def egraph_saturation_with_error_checking(
    ...
    with pipeline_report.nest("Egraph Saturation") as report:
        ...
        if pipeline_debug:
            ...
            report.append("[debug] egglog.extract", egraph.extract(egraph_root))

        # Use egglog's default extractor to get the error messages
        errmsgs = map(lambda x: x.eval(), egraph.extract_multiple(errors, n=10))
        errmsgs_filtered = [
            get_error_message((meth, args)) for meth, args in errmsgs if meth != "root"
        ]
        if errmsgs_filtered:
...
pipeline_middle_end = (
    _compiler_pipeline.trunc("egraph_saturation")
...
def ruleset_propagate_typeof_ifelse(
    ...
    ).then(propagate_ifelse_outs(0, vecports.length(), then_ports, else_ports, ifelse))
    ...
    yield rule(
        ...
        union(TypeVar(ifelse.getPort(idx))).with_(TypeVar(else_ports.getValue(idx))),
    )
    ...
    for attr in attrs.attrs:
        match attr:
            case NbOp_InTypeAttr(idx=int(idx), type=NbOp_Type(name=str(name))):
                ins[idx] = name
            case NbOp_OutTypeAttr(idx=int(idx), type=NbOp_Type(name=str(name))):
                outs[idx] = name
            case _:
                others.append(attr)
...
class Backend:
    ...
    def lower_cast(self, builder, value, fromty, toty):
        match fromty, toty:
            ...
            case _:
                raise NotImplementedError(f"unsupported lower_cast: {fromty} -> {toty}")
        ...
        match expr:
            ...
            case rg.IfElse(cond=cond, body=body, orelse=orelse, operands=operands):
                ...
                for i, (left, right) in enumerate(paired):
                    assert left.type == right.type, f"{left.type} != {right.type}"
...
def ruleset_type_infer_failure_report(
    ...
    yield rule(
        ...
        union(ErrorMsg.root()).with_(
            ErrorMsg.fail(join("Failed to unify if-else outgoing variables: ", name))
        ),
    )
